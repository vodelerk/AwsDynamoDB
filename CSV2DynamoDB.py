    ...
    for i in range(len(csvData)):
        ...
        if i > 0:
            ...
            for v in range(len(csvData[i])):
                ...
                # If a . character is present attempt to cast the value as a float, if this fails, use the index v in the headings list as the key for dataDict and set it equal to the current value of the valu$
                if '.' in value:
                    try:
